...
func setupTest(t *testing.T, fn func(config *Config)) (
	...
	rootConnection, rootClient, _ = newClient(
		config.RootClientCertFile,
		config.RootClientKeyFile,
	)

	var nobodyConnection *grpc.ClientConn
	nobodyConnection, nobodyClient, _ = newClient(
	...
	}}
	{
		...
		for offset, record := range records {
			...
			if res.Offset != uint64(offset) {
				t.Errorf("got offset: %d, want: %d", res.Offset, offset)
			}
		}
	}
	{
...
func testUnauthorized(t *testing.T, _, client api.LogClient, config *Config) {
	...
	if consume != nil {
		t.Errorf("consume response should be nil")
	}
	gotCode, wantCode = status.Code(err), codes.PermissionDenied
	if gotCode != wantCode {
		t.Errorf("got code: %d, want: %d", gotCode, wantCode)
